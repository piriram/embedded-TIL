...
heap이 정말 필요한지와 실시간 제약에서 어떻게 제한해야 하는지는 [실시간 시스템에서 `malloc` 사용 원칙](./100_실시간_시스템에서_malloc_사용_원칙.md)에서 이어서 다룬다.
...
- [전역 변수와 static 변수](./070_전역변수와_static.md) — `.data`·`.bss`와 linkage 복습
- [`sizeof`와 포인터 메모리 크기](./030_sizeof와_포인터_메모리_크기.md) — 포인터 자체와 heap block의 크기 구분
