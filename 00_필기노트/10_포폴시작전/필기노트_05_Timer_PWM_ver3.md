...
<span style="color:#111827">a. CNT가 ARR에 도달 → Update Event 발생</span><br>
...
<span style="color:#111827">b. CCR(Capture/Compare Register)은 CNT와 비교할 값</span><br>
<span style="color:#111827">c. PWM에서 CCR은 HIGH 구간 길이, 즉 duty를 결정</span><br>
<span style="color:#111827">d. 주파수는 timer clock·PSC·ARR, duty는 CCR로 분리해 설명</span><br>
...
<span style="color:#dc2626">★ Timer를 쓴다가 항상 ISR을 쓴다는 뜻은 아님</span><br>
<br>
...
<span style="color:#1d4ed8">① MCU Timer peripheral의 hardware clock이 시간 기준</span><br>
...
<span style="color:#1d4ed8">① RTOS tick이 시간 기준</span><br>
...
<span style="color:#111827">↔ PWM·정밀 timing은 Hardware Timer, RTOS 위의 단순 timeout은 Software Timer</span><br>
<br>
<span style="color:#dc2626">8. 핵심 3줄</span><br>
<span style="color:#dc2626">1) Timer는 PSC → CNT → ARR로 시간을 세고 ARR 도달 시 Update Event를 만든다.</span><br>
<span style="color:#dc2626">2) ISR은 UIF clear와 event 기록만 하고, IMU 읽기·CAN 송신 같은 긴 일은 main/task가 처리한다.</span><br>
<span style="color:#dc2626">3) PWM은 CNT와 CCR의 비교 결과를 hardware가 pin에 직접 반영하며, 주파수는 PSC·ARR, duty는 CCR로 제어한다.</span><br>
<br>
