...
<span style="color:#1d4ed8">1. Timer의 역할</span><br>
<span style="color:#1d4ed8">1) Timer / PWM의 전체 지도</span><br>
<span style="color:#111827">a. busy-wait delay에서는 CPU가 시간이 지났는지 계속 확인함</span><br>
<span style="color:#111827">cf) busy-wait delay = CPU가 반복문을 계속 실행하며 시간·조건을 직접 기다리는 방식</span><br>
<span style="color:#111827">예: `while (시간이_안_지남) { }` 동안 CPU는 다른 일을 처리하지 못함</span><br>
<span style="color:#111827">b. Timer는 peripheral hardware가 clock을 세고, 시간이 되면 event를 만듦</span><br>
<span style="color:#111827">c. CPU는 그 사이 다른 일을 하거나 sleep할 수 있음</span><br>
<span style="color:#dc2626">※ Timer는 단순 delay 함수가 아니라 system의 시간 기준을 제공하는 hardware</span><br>
<br>
<span style="color:#1d4ed8">2) Polling과 Timer interrupt</span><br>
<span style="color:#111827">a. polling: CPU가 timeout을 반복 확인</span><br>
<span style="color:#111827">b. timer interrupt: hardware가 시간 만료를 감지해 CPU에 알림</span><br>
<span style="color:#111827">c. 예: 100 ms마다 센서 읽기, 1초마다 LED toggle</span><br>
<span style="color:#dc2626">※ 긴 delay를 ISR 안에 넣으면 polling 문제가 ISR 안에서 다시 생김</span><br>
<br>
<span style="color:#1d4ed8">3) Timer와 Counter</span><br>
...
<span style="color:#111827">c. 같은 Timer peripheral도 clock source에 따라 두 역할을 할 수 있음</span><br>

---

## 2페이지: Timer는 PSC → CNT → ARR 순서로 시간을 만든다

<span style="color:#1d4ed8">1. Timer의 시간 생성</span><br>
<span style="color:#1d4ed8">1) 하드웨어 시간 흐름</span><br>
<span style="color:#111827">a. Timer clock → PSC → CNT → ARR → Update Event</span><br>
<span style="color:#111827">b. PSC(Prescaler): clock을 나눠 CNT가 세는 속도를 낮춤</span><br>
<span style="color:#111827">c. CNT(Counter): 현재 count 값</span><br>
<span style="color:#111827">d. ARR(Auto-Reload Register): CNT가 셀 끝값</span><br>
<span style="color:#111827">e. CNT가 ARR에 도달하면 reload하고 Update Event를 만들 수 있음</span><br>
...
## 3페이지: Update Event가 ISR로 들어오는 흐름

<span style="color:#1d4ed8">1. Event와 interrupt의 구분</span><br>
<span style="color:#1d4ed8">1) Event가 ISR로 이어지는 흐름</span><br>
<span style="color:#111827">a. CNT가 ARR에 도달하면 Update Event 발생</span><br>
<span style="color:#111827">b. SR의 UIF flag가 event 발생 사실을 기록</span><br>
<span style="color:#111827">c. DIER의 UIE를 켜면 event가 interrupt request가 될 수 있음</span><br>
<span style="color:#111827">d. NVIC가 IRQ를 허용하면 CPU가 handler로 들어감</span><br>
...
<br>
<span style="color:#1d4ed8">3) CPU는 어떻게 돌아오는가</span><br>
<span style="color:#111827">a. interrupt는 main 흐름과 무관하게 발생하는 hardware event</span><br>
<span style="color:#111827">b. CPU는 실행 흐름을 잠시 멈추고 vector table이 가리키는 ISR로 이동</span><br>
<span style="color:#111827">c. ISR이 끝나면 hardware가 저장한 실행 상태를 바탕으로 중단 지점으로 복귀</span><br>
<span style="color:#dc2626">※ 면접에서는 “hardware가 context를 저장·복원한다”까지 설명하면 충분</span><br>

---

## 4페이지: ISR은 알리고, main은 처리한다

<span style="color:#1d4ed8">1. ISR과 main의 역할 분리</span><br>
...
<span style="color:#111827">b. foreground: Timer, GPIO, UART 등의 ISR, background를 잠시 선점</span><br>
<span style="color:#111827">c. 작은 MCU에서는 이 구조를 super loop 또는 main + ISRs라고 부름</span><br>
<br>
<span style="color:#1d4ed8">2) ISR의 원칙</span><br>
<span style="color:#111827">a. UIF를 확인하고 clear</span><br>
<span style="color:#111827">b. flag/event counter 기록처럼 짧은 일만 수행</span><br>
...
<span style="color:#111827">a. ISR이 바꾸고 main이 읽는 flag/counter에는 `volatile`이 필요할 수 있음</span><br>
<span style="color:#111827">b. `volatile`은 compiler가 read/write를 생략하지 않게 할 뿐</span><br>
<span style="color:#111827">c. `volatile`은 atomicity나 race condition 해결책이 아님</span><br>
...
## 5페이지: 포트폴리오 연결 — 100 ms IMU → CAN 흐름

<span style="color:#1d4ed8">1. Timer를 주기 제어에 연결하기</span><br>
<span style="color:#1d4ed8">1) Timer의 목적</span><br>
<span style="color:#111827">a. Timer의 역할은 “100 ms delay를 넣는 것”이 아니라 “100 ms time event를 만드는 것”</span><br>
...
<span style="color:#111827">a. blocking: sensor 읽기 → delay → CAN 송신 → delay</span><br>
<span style="color:#111827">b. non-blocking: Timer가 “시간이 지남”을 event로 전달하고, main loop는 계속 돌아감</span><br>
<span style="color:#111827">c. 장점: UART 수신, 버튼, 오류 event 등 다른 일을 늦게 처리하지 않음</span><br>
...
<span style="color:#111827">b. 실제 `timer_clk`는 보드의 clock tree에서 먼저 확인</span><br>
<span style="color:#111827">c. 예: counter tick을 10 kHz로 만들면 100 ms는 1000 counts</span><br>
<span style="color:#111827">d. 따라서 `ARR + 1 = 1000`, register에는 0부터 세는 +1 규칙 반영</span><br>
<span style="color:#dc2626">※ 다른 STM32의 clock 숫자를 현재 보드에 복사하면 안 됨</span><br>

---

## 6페이지: PWM은 Timer hardware가 출력 파형을 만든다

<span style="color:#1d4ed8">1. PWM의 하드웨어 출력</span><br>
<span style="color:#1d4ed8">1) PWM의 출발점</span><br>
<span style="color:#111827">a. PWM(Pulse Width Modulation)은 digital HIGH/LOW를 빠르게 반복하는 방식</span><br>
<span style="color:#111827">b. 한 주기 중 HIGH 시간의 비율을 duty cycle이라 함</span><br>
...
<span style="color:#111827">d. 주파수는 `timer clock/PSC/ARR`, duty는 `CCR`로 분리해 설명</span><br>
...
<span style="color:#111827">c. duty ≈ `CCR / (ARR+1)`</span><br>
<br>
...
<span style="color:#111827">a. Timer interrupt: ARR 도달 사건을 CPU가 ISR에서 처리</span><br>
<span style="color:#111827">b. PWM: CNT와 CCR 비교 결과를 Timer hardware가 output pin에 직접 반영</span><br>
<span style="color:#111827">c. PWM의 HIGH/LOW 전환마다 CPU가 ISR을 실행할 필요는 없음</span><br>
<span style="color:#dc2626">※ “Timer를 쓴다”가 항상 “ISR을 쓴다”는 뜻은 아님</span><br>

---

## 7페이지: Hardware Timer와 Software Timer는 목적이 다르다

<span style="color:#1d4ed8">1. Timer의 선택 기준</span><br>
<span style="color:#1d4ed8">1) Hardware Timer와 RTOS Software Timer 비교</span><br>
...
<span style="color:#1d4ed8">1. 30초 답변의 핵심</span><br>
<span style="color:#1d4ed8">1) 한 줄 정의</span><br>
<span style="color:#111827">a. Timer는 peripheral clock을 세어 시간 event를 만들고, PWM은 그 Timer의 비교 기능으로 duty가 있는 출력 파형을 만드는 hardware 기능이다.</span><br>
<span style="color:#1d4ed8">2) 왜 필요한가</span><br>
<span style="color:#111827">a. delay loop로 CPU를 묶지 않고 일정한 시점에 일을 시작하며, PWM은 CPU가 pin을 반복 toggle하지 않아도 되게 한다.</span><br>
<span style="color:#1d4ed8">3) 동작</span><br>
<span style="color:#111827">a. Timer는 `PSC → CNT → ARR` 흐름으로 count하고 ARR 도달 시 Update Event를 만든다.</span><br>
<span style="color:#111827">b. interrupt를 켜면 ISR은 flag를 clear하고 event만 남기며, sensor 읽기·CAN 송신 같은 긴 일은 main이 처리한다.</span><br>
<span style="color:#111827">c. PWM은 CNT와 CCR을 비교해 output pin을 hardware가 직접 바꾼다.</span><br>
<span style="color:#1d4ed8">4) 비교</span><br>
<span style="color:#111827">a. Timer interrupt는 CPU가 event를 처리하는 방식이고, PWM은 hardware가 pin 출력을 직접 만드는 방식이다.</span><br>
<span style="color:#111827">b. Software Timer는 RTOS tick 기반 timeout 기능이라 Hardware Timer와 목적이 다르다.</span><br>
<span style="color:#1d4ed8">5) 30초 통합 답변</span><br>
<span style="color:#111827">a. Timer는 peripheral clock을 PSC로 분주해 CNT를 세고, ARR에 도달하면 Update Event를 만드는 하드웨어입니다. interrupt를 사용하면 ISR에서는 UIF clear와 event 기록만 짧게 수행하고, IMU 읽기나 CAN 송신은 main loop로 넘겨 다른 event를 막지 않게 합니다. PWM은 같은 Timer에서 CNT와 CCR을 비교해 하드웨어가 pin의 HIGH/LOW 시간을 직접 바꾸므로, 주파수는 PSC와 ARR, duty는 CCR로 조절할 수 있습니다.</span><br>
